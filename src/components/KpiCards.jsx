...
export default function KpiCards() {
    ...
    const totalPruchases = purchases.reduce((acc, item)=> acc + Number(item.price_total), 0);
    const totalProfit = totalSales - totalPruchases;
        ...
        <Card key={item.title} decoration="top" decorationColor={item.color}>
          <Flex justifyContent="start" className="space-x-4">
            ...
            <div className="flex-1 pl-3">
              ...
              <Metric className="">{item.metric}</Metric>
            </div>
          </Flex>
        </Card>
      ))}
